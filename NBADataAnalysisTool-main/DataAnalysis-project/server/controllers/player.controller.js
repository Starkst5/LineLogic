...
// Search current players (existing)
  ...
  const sql = `
    ...
    next_games AS (
      SELECT DISTINCT ON (pt.person_id)
        pt.person_id,
        CASE
          WHEN s.home_team_id = pt.team_id THEN s.away_team_id
          ELSE s.home_team_id
        END AS opponent_team_id,
        CASE
          WHEN s.home_team_id = pt.team_id THEN away_t.team_abbrev
          ELSE home_t.team_abbrev
    ...
    GROUP BY p.person_id, p.first_name, p.last_name, ng.opponent_abbr, ng.opponent_team_id
...
// Get a player's game stats (last 10 games)
export async function getPlayerGames(pool, playerId) {
    const sql = `
      SELECT
        ...
        g.minutes,
        g.opponent_team_id,
        opp.team_abbrev AS opponent_abbr
...
// Get context for a player's game today (home/away, opponent team)
export async function getPlayerGameContext(pool, playerId) {
    const sql = `
      ...
      SELECT
        ...
        s.game_datetime_est,
        CASE
          WHEN s.home_team_id = pct.team_id THEN true
          ELSE false
        END AS is_home,
        CASE
          WHEN s.home_team_id = pct.team_id THEN away_team.team_abbrev
          ELSE home_team.team_abbrev
        END AS opponent_abbr,
        CASE
...
export async function getPlayersByTeam(pool, teamId) {
  const sql = `
    WITH current_season AS (
      ...
      FROM public.player_game_stats
      WHERE team_id = $1
        AND game_datetime_est >= '2025-10-21'
      GROUP BY person_id
    )
    SELECT
      p.person_id,
...
export async function getOpponentDefByPosition(pool, opponentTeamId, playerId) {
  const sql = `
    ...
    SELECT
      ...
      vo.avg_ftm_allowed,
      vo.sample_size,
      la.avg_pts    AS league_avg_pts,
      la.avg_reb    AS league_avg_reb,
      la.avg_ast    AS league_avg_ast,
      la.avg_blk    AS league_avg_blk,
      la.avg_stl    AS league_avg_stl,
      la.avg_3pm    AS league_avg_3pm,
